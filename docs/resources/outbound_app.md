...
- `tenant_id` (String) The ID of the tenant that owns this outbound application, for applications scoped to a single tenant rather than the whole project. An empty value (the default) creates a project level application. This value cannot be changed after creation, so changing it will require the resource to be deleted and recreated.
