...
- `tenant_user_isolation` (Boolean) When enabled, users are completely isolated per tenant. The same email address in Tenant A and Tenant B will be treated as separate identities with isolated credentials, sessions, and MFA state.
