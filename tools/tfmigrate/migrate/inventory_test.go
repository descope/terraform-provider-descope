...
func TestMigrationLifecycle(t *testing.T) {
	...
	assert.Contains(t, readme, "Reconnect temporary literals for retained core attributes")
	assert.Contains(t, readme, "`manifest.json` entry whose `legacy_path` owns it")
	assert.Contains(t, readme, "never reconnect")
	stage2Start := strings.Index(readme, "# Stage 2:")
	require.GreaterOrEqual(t, stage2Start, 0)
	stage2 := readme[stage2Start:]
	require.Contains(t, stage2, "terraform init -upgrade")
	require.Contains(t, stage2, "terraform validate")
	assert.Less(t, strings.Index(stage2, "terraform init -upgrade"), strings.Index(stage2, "terraform validate"))
