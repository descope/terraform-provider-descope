...
resource "descope_user_attribute" "text" {
  # attribute creation is serialized with depends_on: parallel creates race on the
  # backend's shared attributes record ("attributes entity already exists"), see
  # finding #13 in the findings log
  project_id = descope_project.test.id
  id         = "fuzzText"
  name       = "Text Attribute"
  type       = "string"
