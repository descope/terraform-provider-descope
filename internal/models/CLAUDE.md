...
- `setattr` - Set of nested objects/models
- `jsonattr` - A JSON document stored as a string, compared semantically
...
- `.Optional()` - Unknown on create unless specified, then keeps the stored value (see the traps in the root CLAUDE.md)
...
- Implement `ModifyPlan()` for cross-attribute plan logic (see Add Plan Logic below)
...
This function is called to build the request body sent to the resource's endpoint. The JSON key passed to the
`Get` function as the third parameter must match the field name in the endpoint's payload.
...
we want to update the model struct with the data from the endpoint's response.
...
On the resource's top-level model, implementing `Validate()` is enough: the resource calls it during validation. A
nested model needs a validator object, created above its `*Attributes` map in the same file:
...
For cross-attribute logic at plan time, implement `ModifyPlan()` on the resource's top-level model. The resource
calls it after the attribute plan modifiers on create and update plans, never on destroy, with the planned model as
the receiver to mutate in place:

```go
func (m *ExampleModel) ModifyPlan(h *helpers.Handler, config *ExampleModel, state *ExampleModel) {
    // m is the plan and might contain Unknown values, state is a zero-value model (all nulls) on create
}
```

See `settings/session.go` for an example that rejects an invalid combination of values.
...
For collections whose order the backend doesn't preserve, use a set rather than a list so a reordered response
doesn't show as a change: `strsetattr` for plain strings, and `setattr` for nested models.

## Common Pitfalls

You can check existing models to see if you're missing anything, but common mistakes are:

- Forgetting to add the field to all four places (Attributes, Struct, Values, SetValues)
- Mismatched `tfsdk` tag and Attributes map key
- Wrong JSON key name in Values/SetValues (check it against the response of the resource's read endpoint)
...
3. Run specific acceptance tests by supplying the test name, e.g., `make testacc tests=TestPermission`
...
- **Simple attributes**: `permission/permission.go`
- **String sets**: `role/role.go`
- **Durations with defaults and plan logic**: `settings/session.go`
- **Lists of nested models**: `settings/adminportal.go`
- **Nested model validators**: `connectors/shared.go`
